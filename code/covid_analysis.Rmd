...
packages(c("readxl", "readr", "tidyverse" , "data.table" , "naniar", "rockchalk", "lubridate", "kableExtra", "survival", "formatR", "condSURV", "janitor", "sjlabelled", "Hmisc", "rms", "aod", "survminer", "glmnet", "tableone", "broom", "cmprsk", "comorbidity", "tidycmprsk", "tidymodels", "ggsurvfit", "investr"))
```
...
# **Table 2**
#### **Demographics by Paxlovid treatment status**
...
{
  formula <- as.formula(paste("paxlovid ~ ", varlist[i])) # Use paste to create formula for model
  glmmodel <- glm(formula, df_reg_pax, family = poisson(link = "log")) # Run model
  est <- as.matrix(coeftest(glmmodel, vcov = sandwich))
  term <- as.matrix(names(coef(glmmodel)))
  colnames(term) <- "term"
  estimate <- as.matrix(round(exp(est[,1]),2)) # Extract estimate from model results
  colnames(estimate) <- "rr"
  lower <- as.matrix(round(exp(est[,1] - 1.96 * est[,2]),2)) # Calculate confidence interval
  colnames(lower) <- "lower"
  upper <- as.matrix(round(exp(est[,1] + 1.96 * est[,2]),2))
  colnames(upper) <- "upper"
  df <- cbind(term, estimate, lower, upper)
  rr_unadj <- rbind(rr_unadj, df) |>
...
hr_unadj |>
  ...
  kable("html", caption = "Unadjusted hazard ratios") |>
  kable_styling()
```
...
hr_ageadj |>
  ...
  select(-c(estimate, std.error, statistic, p.value)) |>
  kable("html", caption = "Age-adjusted hazard ratios") |>
  kable_styling()
```
