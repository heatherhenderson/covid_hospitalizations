...
```{r message=FALSE, warning=FALSE}
# Full model
td_dat <- tmerge(data1 = df_hr1 |> select(pat_mrn_id, ftime, fstatus, gend_male, age_at_dx, age_grp2, vax_doses, full_vax, race_eth2, study_wk, ccs2, ses_qrtile, low_ses),
...
full_fg_mod <- crr(Surv(ftime, fstatus) ~ strata(pax_treat) + age_grp2 + gend_male + race_eth2 + full_vax + ccs2 + ses_qrtile + rcs(study_wk, 3) + ccs2, data = td_dat)
