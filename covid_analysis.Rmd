...
for (i in seq_along(varlist)) # Iterate over cols_covar vector
{
  ...
  lmmodel <- lm(formula, df_reg_rd)
  est <- as.matrix(coeftest(lmmodel))
  term <- as.matrix(names(coef(lmmodel)))
  colnames(term) <- "term"
  estimate <- as.matrix(round(est[,1],5))
  colnames(estimate) <- "rd"
  lower <- as.matrix(round(est[,1] - 1.96 * est[,2],5))
  colnames(lower) <- "lower"
  upper <- as.matrix(round(est[,1] + 1.96 * est[,2],5))
  colnames(upper) <- "upper"
  df <- cbind(term, estimate, lower, upper)
  rd_unadj <- rbind(rd_unadj, df) |>
    filter(term != "(Intercept)",
           rd != 0) |>
    remove_rownames()
}
...
```{r message=FALSE, warning=FALSE}
# Fully adjusted
lmmodel <- lm(covid_hosp ~ paxlovid + full_vax + gender + age_grp2 + race_eth2 + ses_qrtile + ccs2, df_reg_rd)
est <- as.matrix(coeftest(lmmodel))
term <- as.matrix(names(coef(lmmodel)))
colnames(term) <- "term"
estimate <- as.matrix(round(est[,1],5))
